...
public class MiningPlanet extends jason.environment.Environment
{
  ...
  private void updateAgPercept(String agName, int ag)
  {
    ...
    if ((model.getAgIdBasedOnName(agName)==0) || (model.getAgIdBasedOnName(agName)==3))
     dosah=3*model.agSuperability[model.getAgIdBasedOnName(agName)];   
  ...
  private void updateAgPercept(String agName, int x, int y)
  {
    ...
    {
      if (model.hasObject(WorldModel.SPECTACLES, x, y)) 
      {
        addPercept(agName, Literal.parseLiteral("spectacles(" + x + "," + y + ")"));
      }
      if (model.hasObject(WorldModel.SHOES, x, y)) 
      {
        addPercept(agName, Literal.parseLiteral("shoes(" + x + "," + y + ")"));
      }
