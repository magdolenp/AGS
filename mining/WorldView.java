...
public class WorldView extends GridWorldView
{
  ...
  public void draw(Graphics g, int x, int y, int object)
  {
    switch (object)
    {
      ...
      case WorldModel.SHOES:   drawShoes(g, x, y); break;
      case WorldModel.GLOVES:  drawGloves(g, x, y); break;
      case WorldModel.SPECTACLES: drawSpectacles(g, x, y); break;
    }
  }
  ...
  public void drawAgent(Graphics g, int x, int y, Color c, int id)
  {
    ...
    int count = 3+((WorldModel)model).carryingGoldGet(id) + ((WorldModel)model).carryingWoodGet(id);
...
//FZjr
  ...
  {
    g.setColor(Color.green);
    g.drawOval( x*cellSizeW-3, y*cellSizeH-3, x*cellSizeW+3, y*cellSizeH+3);
  }
